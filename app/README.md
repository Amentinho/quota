...
A fourth tab, `src/components/DemoView.tsx`, calling the local server in `server/` (`http://localhost:4317`) — three cards: mint as certifier (editable grams), revoke/grant the `MINTER` role, and transform (editable grams in and grams claimed out, pre-filled 1000g → 450g for a one-click happy path). The transform card reads `quota.yield.kernel.bp` live from ENS once on mount (`lib/ens.ts`'s `readTextRecord`) and recomputes `floor(input × yieldBp / 10000)` on every keystroke, displayed next to the fields — so the ceiling is visibly derived from the live ratio, not a number that happens to match whatever's hardcoded into a button label. Every response renders a headline plus the actual raw payload underneath (the real revert string, status code, or transaction hashes) — the credibility is the unparaphrased error, not a friendly summary.
