...
- **Solvency** (`src/components/SolvencyView.tsx`) — per consortium, per season: cap, minted, retired, in circulation, plus a proportional utilization bar (not a grouped bar chart — at real harvest scale, a handful of demonstration grams against a 3,400-tonne cap would render as invisible slivers next to the cap bar; a stacked share of the cap stays legible at any ratio). Detector findings (`unauthorizedMints`, `unauthorizedRetirementOutflows`) are rendered directly under the season they belong to, not behind a separate tab.
