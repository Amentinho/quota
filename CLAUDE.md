...
- One shared subgraph schema indexing multiple consortia through a single query pattern, backed by a real Subgraph Studio deployment (project slug: `quota`) — no mocked data. Schema and mappings built and verified (see Architecture, Status); actual `graph deploy` pending a Studio deploy key.
...
- Subgraph: `schema.graphql` (six required entities + two detector entities, all consortium-scoped) and `src/mapping.ts` written, `graph codegen`/`graph build` both succeed against the v6 contract. Mint reconciler (`scripts/relayer.mjs reconcile-mints`) built, both real bugs it surfaced fixed (see "Bridging Hedera mirror-node data" above), and verified clean: 1 legitimate relayer mint (anchored) + 1 deliberate bypass mint via `scripts/mint-bypassing-relayer.mjs` (no anchor, simulating real unauthorized minting) → reconciler flagged exactly the bypass mint, correctly recognized every legitimate mint across three historical contract addresses. Not yet done: actual `graph deploy` to Subgraph Studio (needs a deploy key — see Open decisions) and the three example queries haven't been run against a live endpoint (written and schema-valid, untested against real deployed data).

### Not yet built
- Subgraph deployment to Studio itself — schema, mappings, and the detector are built and verified locally; `graph deploy` needs a Studio deploy key (browser wallet connection, not something this session can do — see Open decisions).
...
- **Blocking the subgraph deploy**: need a Subgraph Studio deploy key for a project named `quota`. Created by connecting a wallet in Studio's browser UI — the user's action, not something this session can do. Once available: `graph auth <key>` then `graph deploy quota` from `subgraph/`.
- Once deployed, the three example queries in the README need to actually be run against the live endpoint and their real output substituted in for whatever placeholder is there — don't leave them as "should work" without having actually run them.
