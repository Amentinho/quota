...
- One shared subgraph schema indexing multiple consortia through a single query pattern, backed by a real Subgraph Studio deployment (project slug: `quota`) — no mocked data. Live at `https://api.studio.thegraph.com/query/1758548/quota/v0.0.1`, synced to chain head, all three example queries run against it for real (see Status).
...
`QuotaAnchor.sol` — deployed and verified (Etherscan, Blockscout, Sourcify): [`0x39F0Fded796cB5323048a15b907CcE38979EDa2c`](https://sepolia.etherscan.io/address/0x39F0Fded796cB5323048a15b907CcE38979EDa2c#code) (`QUOTA_ANCHOR_ADDRESS` in `.env`, current -- this is v6, the only address the subgraph and every live link in README point at; see Status below for the full deployment history and why five earlier addresses are deprecated, three of them holding immutable false-positive detector events that must never be linked as if current). Stores almost nothing: the immutable `relayer` address (`onlyRelayer` modifier, set once at construction, never rotatable — without this anyone could emit fabricated anchor events for Hedera transaction IDs that never happened), plus one small struct per season (`seasonEns` mapping: parent registry, label, resolver, node) recording where to look on ENS, set once at `openSeason` and read — never re-supplied by the caller — on every later `recordMint`. Everything else is emit-only. Events:
```
...
`subgraph/schema.graphql`: six required entities (`Consortium`, `Season`, `Participant`, `Lot`, `Transformation`, `Retirement`) plus two detector entities (`UnauthorizedMint`, `UnauthorizedRetirementOutflow`), all consortium-scoped by construction. `Season`'s `mintedGrams`/`retiredGrams`/`inCirculationGrams`/`utilisationBp` are computed in `subgraph/src/mapping.ts` on every relevant event (`recomputeSeasonAggregates`), not left to query-time aggregation. `Lot` aggregates every `Retirement` sharing a `(seasonId, lotRef)` pair, since a lot can be retired against more than once as it sells through.

Scaffolded by hand (`graph-cli`/`graph-ts` installed directly, manifest/schema/mappings hand-written) rather than `graph init`, to avoid its interactive prompts in a non-interactive environment — same reasoning as avoiding `hardhat --init` earlier. `graph codegen` and `graph build` both succeed against the final contract. One real gotcha hit immediately: this graph-cli version requires an explicit `@entity(immutable: true|false)` on every entity — `@entity` alone fails codegen outright (`@entity directive requires 'immutable' argument`), not documented anywhere obvious, found from the actual error message.

`UnitsTransferred` is not indexed — it doesn't map to any of the six required entities and nothing yet needs it queryable. `ParticipantRegistered` was found missing `consortiumId` (address+role only) while writing the schema — without it, `Participant` couldn't be scoped to a consortium, breaking the composability claim specifically for participants. Fixed before the schema was written against it, in the same redeploy cycle as the detector event, not as a separate round.

The mint-reconciliation detector and its two real bugs are covered in full under "Bridging Hedera mirror-node data into the subgraph" above.
...
- Layer 2: `QuotaAnchor.sol`, six redeploys, each for a real reason — full history kept in `.env`, not cleaned up, because the subgraph's mint reconciler needs every address to check history against (see "Bridging Hedera mirror-node data" above):
  ...
  - **v6 (current)** `0x39F0Fded796cB5323048a15b907CcE38979EDa2c` = `QUOTA_ANCHOR_ADDRESS`, deployment block in `QUOTA_ANCHOR_DEPLOY_BLOCK` — clean, both reconciler bugs fixed before this one was tested. This is what `subgraph.yaml` points at.
...
- Subgraph: `schema.graphql` (six required entities + two detector entities, all consortium-scoped) and `src/mapping.ts` written, `graph codegen`/`graph build` both succeed against the v6 contract. Deployed for real to Subgraph Studio (`npx graph deploy quota`, no `--studio` flag on this graph-cli version — plain positional subgraph name is enough once `graph auth` has set the deploy key): **`https://api.studio.thegraph.com/query/1758548/quota/v0.0.1`**. `_meta { block { number } hasIndexingErrors } }` confirmed `hasIndexingErrors: false` and the indexed block tracking chain head within a few blocks throughout.
  - Mint reconciler (`scripts/relayer.mjs reconcile-mints`) built, both real bugs it surfaced fixed (see "Bridging Hedera mirror-node data" above), and verified clean: 1 legitimate relayer mint (anchored) + 1 deliberate bypass mint via `scripts/mint-bypassing-relayer.mjs` (no anchor, simulating real unauthorized minting) → reconciler flagged exactly the bypass mint, correctly recognized every legitimate mint across three historical contract addresses.
  - Query 3 (retirement traceability) came back empty on first run against the live endpoint — `registerParticipant`/`recordRetirement` were real contract functions the relayer never actually called (see "Not yet built", now resolved below). Rather than ship an empty result, added `retire` and `register-participant` commands to `scripts/relayer.mjs` and ran them for real: `register-participant 0x152415948666860DcF4E0cDBd867cc61D2DaEDC7 grower` (a freshly generated address, no stored private key — same "no key needed, QuotaAnchor only checks role/identity, never asks for a signature from it" pattern as `ENS_CERTIFIER_ADDRESS`); `retire 1 lot-2026-001`, which does a real Hedera `TransferTransaction` of 1 gram from the operator to the retirement account first, then anchors `recordRetirement` with that transfer's real Hedera tx ID — same do-the-Hedera-leg-first pattern as `doMint`, not a fabricated anchor.
  - All three README queries run against the live endpoint afterward and returned real, non-empty JSON: query 1 shows `mintedGrams: "1"`, `retiredGrams: "1"`, `inCirculationGrams: "0"`; query 2 shows the bypass mint (`hederaTxId: "0.0.10323351-1788884980-415990532"`) under `unauthorizedMints`, `unauthorizedRetirementOutflows: []`; query 3 shows the `grower` participant and `lot-2026-001` with its one real retirement. Actual returned JSON is in the README, not just the query text.

### Not yet built
- Relayer operations for `transfer` and `transform` — contract functions exist, relayer doesn't call them yet. (`retire` and `registerParticipant` are now wired up — see Status.)
...
- Subgraph is deployed and live; both items above are resolved. Remaining open item: `transfer` and `transform` are still not wired into the relayer, so `UnitsTransferred`/`Transformed` have no real on-chain data yet, and the subgraph's `Transformation` entity is untested against a real event (schema/mapping are written, just never triggered).
